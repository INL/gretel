...
import { distinct, distinctUntilChanged, map, mergeMap } from 'rxjs/operators';
...
export class TreebankSelectionService {
    get state$(): Observable<TreebankSelection> {
        return this.stateService.state$.pipe(
            map(state => state.selectedTreebanks), 
            distinctUntilChanged((a, b) => !a.equals(b))
        );
    }

    get selectedTreebanksLoaded$(): Observable<Array<{treebank: TreebankLoaded, selectedComponents: TreebankComponent[]}>> {
        return this.state$.pipe(
            distinctUntilChanged((a, b) => a.),
            mergeMap(selectedTreebanks => selectedTreebanks.selectionLoaded())
        );
    }
