...
namespace Django {
    export interface TreebankResponse {
        slug: string;
        title: string;
        description: string;
    ...
    export interface TreebankMetadataResponse {
        field: string;
        type: 'text' | 'int' | 'date';
    ...
    export type ComponentsForTreebankResponse = {
        slug: string;
        title: string;
        description: string;
        nr_sentences: number;
    ...
    export function makeMetadata(item: TreebankMetadataResponse): TreebankMetadata {
    ...
    export function makeComponent(comp: ComponentsForTreebankResponse): TreebankComponent {
        return {
    ...
    */
    export const makeComponents = (treebank: TreebankStub, componentsFromServer: ComponentsForTreebankResponse[]): 
        ...
        */
        ...
        for (const c of componentsFromServer.map(makeComponent)) {
            components[c.id] = c;
    ...
    export const makeTreebank = (bank: TreebankResponse): TreebankStub => ({
        provider: 'gretel',
        id: bank.slug,
...
export class TreebankService {
    ...
    private progressivelyEnhanceTreebank = (treebank: TreebankStub): Observable<TreebankLoaded> => {
        ...
        const metadata$: Observable<TreebankMetadata[]> = 
            ...
            .pipe(
                mergeMap(url => this.http.get<{'metadata': Django.TreebankMetadataResponse[]}>(url)),
                map(r => r.metadata.map(Django.makeMetadata)),
            );

        // Fetch components and put in treebank when they come in.
        // Then re-emit the treebank
        const components$: Observable<ReturnType<typeof Django.makeComponents>> = 
            from(this.configurationService.getDjangoUrl('treebanks/treebank/' + treebank.id + '/components/'))
            .pipe(
                mergeMap(url => this.http.get<Django.ComponentsForTreebankResponse[]>(url)),
                map(c => Django.makeComponents(treebank, c)),
            )
    ...
    constructor(
        ...
        .pipe(
            ...
            scan((acc: Record<string, Treebank>, treebank) => Object.assign(acc, {[`${treebank.provider}_${treebank.id}`]: treebank}), {}),
